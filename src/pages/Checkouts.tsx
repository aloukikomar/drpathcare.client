                  ...
                  <div key={it.id} className="border rounded-lg p-4 shadow-sm bg-white">
                    ...
                    <div className="flex flex-col md:flex-row md:justify-between gap-4">
                      <div>
                        ...
                        <div className="text-sm text-gray-700 mt-1">
                          Price: ₹{it.offer_price ?? it.base_price ?? it.price}
                        </div>
                      </div>
                      ...
                      <div className="w-full md:w-80 flex flex-col gap-2">
                        ...
                        <div className="relative">
                          <select
                            ...
                            className="w-full appearance-none bg-gray-50 border border-gray-300 rounded-lg px-3 py-2 text-sm pr-10 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            defaultValue=""
                          >
                            <option value="">Select patient</option>
                            {patientsList.map((p) => (
                              <option key={p.id} value={p.id}>
                                {p.first_name}{p.last_name ? ` ${p.last_name}` : ""} | {p.gender} | {p.age}{p.mobile?` | ${p.mobile}`:""}
                              </option>
                            ))}
                          </select>
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() => {
                              ...
                              if (!sel) return toast.error("Pick a patient to assign");
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      {(assignedPatients[String(it.id)] || []).map((p: any) => (
                        <div
                          key={p.id}
                          className="flex items-center gap-2 bg-blue-50 border border-blue-200 px-3 py-1.5 rounded-full text-sm shadow-sm"
                        >
                          {/* Avatar Circle */}
